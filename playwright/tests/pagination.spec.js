...
import { contactsTotal, waitForContactsLoaded } from "./helpers.js";

test("shows adaptive pagination and navigates between pages", async ({ page, request }) => {
  test.skip((await contactsTotal(request)) < 60, "Pagination window requires a sufficiently large dataset");
...
test("resets and recalculates pagination when search changes", async ({ page }) => {
  await page.goto("/contacts");
  await waitForContactsLoaded(page);
